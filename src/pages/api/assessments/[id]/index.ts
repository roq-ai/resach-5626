...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    ...
    case 'PUT':
      return updateAssessmentById();
    case 'DELETE':
      return deleteAssessmentById();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getAssessmentById() {
    const data = await prisma.assessment.findFirst(convertQueryToPrismaUtil(req.query, 'assessment'));
    return res.status(200).json(data);
  }

  async function updateAssessmentById() {
    await assessmentValidationSchema.validate(req.body);
    const data = await prisma.assessment.update({
  ...
  async function deleteAssessmentById() {
    const data = await prisma.assessment.delete({
      where: { id: req.query.id as string },
    });
    return res.status(200).json(data);
  }
