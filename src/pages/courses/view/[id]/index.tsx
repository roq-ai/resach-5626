...
import { deleteModuleById } from 'apiSdk/modules';
...
function CourseViewPage() {
  ...
  const moduleHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('module', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/modules/create?course_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.module?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/modules/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.title}</Td>
